...
class VirtualTemperatureConnectivity(LinearSteadyStatePDEConstraint):
    ...
    The penalty is the mean source-cell temperature, **normalized by the
    fully-disconnected baseline** (the same solve with
    :math:`\\kappa \\equiv \\kappa_\\min` everywhere, i.e. :math:`\\rho = 0`).
    That baseline depends only on the masks / grid / ``kappa_min`` — not on
    ...
    def penalty(
        ...
        temp_mean_source = jnp.sum(u * src) / src_count

        # Fully-disconnected baseline: the same heat solve with κ ≡ κ_min
        # everywhere (equivalently ρ = 0).  It depends only on the masks /
        # grid / κ_min, so it is a constant normalizer — solved here and
        # stop-gradient'd.  The connectivity CG is ≪ the FDTD cost, so the
        # extra solve is negligible.  Normalizing makes the raw penalty
        # intrinsically ~[0, 1]: ≈1 fully disconnected, →0 well connected.
        kappa_ref = jnp.full(
            self.source_mask.shape,
            jnp.asarray(self.kappa_min, dtype=u.dtype),
        )
        t_ref = _solve_heat_eq(
            kappa_ref,
            self.source_mask.astype(jnp.float32),
            drain,
            cg_iterations=self.cg_iterations,
            cg_tol=self.cg_tol,
        )
        ref = jnp.sum(t_ref * src) / src_count
        ref = jax.lax.stop_gradient(jnp.maximum(ref, jnp.asarray(1e-30, dtype=u.dtype)))
        p = temp_mean_source / ref
        info = {
            f"{self.name}_temp_max": u.max(),
            f"{self.name}_temp_mean_source": temp_mean_source,
            f"{self.name}_disconnect_ref": ref,
            f"{self.name}_penalty_norm": p,
        }
        return p, info
