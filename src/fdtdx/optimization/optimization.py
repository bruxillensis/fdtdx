...
class Optimization(TreeClass):
    ...
    Attributes:
        ...
        epoch_callback: Optional ``callable(**kwargs)`` invoked every logged
            epoch (``epoch``, ``params``, ``objects``, ``arrays``, ``info``,
            ``optimization``) for live monitoring / status plots. Exceptions
            inside it are caught and logged, never fatal.
    """
    ...
    # Optional user hook invoked on every logged epoch (gated by
    # ``log_every``) AFTER params/detectors are logged, called as
    # ``epoch_callback(epoch=int, params=params, objects=self.objects,
    # arrays=arrays, info=info, optimization=self)``.  Use it to draw a live
    # status page, push metrics, early-stop bookkeeping, etc.  Exceptions
    # raised inside it are caught and logged (a broken monitor must never
        ...
        *,
        ...
        for epoch in range(start_epoch, self.total_epochs):
            ...
            if self.logger is not None and epoch % self.log_every == 0:
                # Log the design *as fabricated* (projected at a hardened β).
            ...
            if self.epoch_callback is not None and epoch % self.log_every == 0:
                # A broken live monitor must never kill a long optimization.
                try:
                    self.epoch_callback(
                        epoch=epoch,
                        params=params,
