...
that are jointly optimized via gradient descent. The figure of merit is a weighted
sum of two mode-power objectives (both normalized to [0,1]):
(a) downward Gaussian mode power: fraction of input flux directed downward × coherent
    Gaussian overlap (phasor is angular-spectrum-propagated to the focal plane), and
(b) output waveguide mode power: fundamental mode overlap at the output waveguide.
"""
...
def main(
    ...
    # Objective weights
    # ------------------------------------------------------------------
    radiation_fraction = 0.5  # target fraction of input power into downward Gaussian mode
    ...
    def loss_func(
        ...
        # Two mode-power objectives, both in [0, 1]:
        #   obj_out:  fraction of input fundamental mode power in output waveguide mode
        #   obj_down: fraction of input power going downward × Gaussian shape match
        obj_out = flux_efficiency_out
        obj_down = flux_efficiency_down * overlap
        objective = (
            (1.0 - radiation_fraction) * obj_out
            + radiation_fraction * obj_down
            - fab_weight * fab_penalty
        )
