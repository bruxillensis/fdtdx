...
**`pre_step_hook(*, epoch, params, arrays, objects, optimization) → updated_arrays | None`** runs *before* each compiled step, with full Python freedom (host-side numpy/scipy work, file I/O, …). Use it to thread **out-of-trace** computations into the JIT region by mutating `arrays.detector_states` and returning the updated container (host-side numpy/scipy results, file I/O, externally-computed control values). It is the escape hatch for genuinely **non-differentiable** out-of-trace work; anything differentiable belongs directly in `simulate_fn`. Returning `None` keeps arrays unchanged. **Exceptions are NOT caught** — a broken pre-step is a hard error because it can desynchronise the gradient. Any keys it writes must be pre-seeded with zero arrays in the initial `arrays` before constructing `Optimization`, else the first epoch fails on pytree mismatch.
...
### Multi-state / split-pass optimization (`fdtdx.MultiStateOptimization`, `fdtdx.accumulated_value_and_grad`)

When a loss couples **several independent simulations that don't fit in memory together** (multi-wavelength, multi-angle / incidence, fabrication-corner robustness, or any set of perturbed/actuated states), the single-`simulate_fn` `Optimization` driver holds every gradient tape at once. The fix is a **memory-bounded gradient-accumulation** primitive: because the loss is a function of small per-state metrics, `dL/dρ = Σ_i (∂combine/∂m_i)·(dm_i/dρ)`, so the states are differentiated **one at a time and summed** — peak memory ≈ one sim, runtime ≈ Nx.

- **`accumulated_value_and_grad(params, state_fns, combine)`** — the bare primitive. `state_fns` is `{name: f(params) → metric_pytree}` (each runs ONE sim → a *small* metric); `combine` is `{name: metric} → (scalar_loss, info)`. Returns `(loss, grad, info, metrics)`, matching a single `value_and_grad` over the combined loss (incl. complex/phase metrics). Pure JAX — no FDTD assumption.
- **`MultiStateOptimization`** — a driver mirroring `Optimization.run` (optax / logger / checkpoint / seed / resume) that pre-jits each state's forward + vjp once and accumulates. Fields: `state_fns={name: f(params, key, epoch) → (metric, aux)}`, `combine`, `optimizer`, `params`, optional `objects` (logging only), `total_epochs`, `param_clip`, `logger`, … It does **not** thread `arrays` across epochs (no donation), so unlike `Optimization` there's no reserved-`detector_states`-key seeding constraint — but each per-state vjp runs **through `run_fdtd`**, so `config` needs a `gradient_config` (reversible recorder).

Each `state_fn` is just a `params → metric` callable, so a state's simulation can be anything — different sources, wavelengths, incidence angles, or perturbed designs. Use it for multi-wavelength / multi-angle / fabrication-corner-robustness objectives. (A companion package may supply adapters that turn its own externally-actuated states into `state_fn`s; the concrete loss recipe lives in the application layer.)
...
- **`simulate_fn` must return a structurally identical `ArrayContainer` every epoch** — the driver compiles `loss_fn` once and threads the output back as the next input. Schemes that add extra `detector_states` keys must pre-seed those keys (zeros) into the initial `arrays` *before* building `Optimization`, else epoch 1 fails with a pytree mismatch. (`MultiStateOptimization` is exempt — it does not thread `arrays` across epochs.)
- **`config.time` must be long enough for light to traverse the whole device plus settle** — too short and the output detectors read a tiny precursor that's near-insensitive to the design, starving the gradient.
- **`ModeOverlapDetector.compute_overlap` needs `.apply(...)` called once at setup** to populate its reference mode; the optimization driver passes pre-`apply` objects to metric callbacks, so close your metric over a setup-prepared detector (not the one inside `objects`).
